<x-app-layout>
    <div class="w-full md:w-1/2 p-4 mx-auto bg-white rounded">

        <!--レシピ詳細-->
        <div class="rounded border border-gray-500 mt-4">
            <div class="aspect-w-1 aspect-h-1 max-w-xs mx-auto">
                <img class="object-cover w-full h-full rounded-t-lg" src="{{ asset($recipe->image) }}" alt="{{$recipe['title']}}">
            </div>
            <div class="p-4">
                <p class="text-2xl font-bold mb-2 mt-4">{{$recipe['title']}}</p>
                <p class="mb-4">{{$recipe['description']}}</p>
                <h4 class="text-2xl font-bold mb-2 mt-4">材料</h4>
                @foreach($recipe['ingredients'] as $i)
                <ul class="text-gray-500 ml-6">
                    <li>{{$i['name']}}:{{$i['quantity']}}</li>
                </ul>
                @endforeach    
            </div>
        </div>

        <!-- 作り方 -->
        <div class="mt-4">
            <h4 class="text-2xl font-bold mb-2">作り方</h4>
            @foreach($recipe['steps'] as $s)
                <div class="flex items-center mb-2">
                    <div class="w-10 h-10 flex items-center justify-center bg-gray-200 rounded-full mr-4 mb-2">
                        {{ $s['step_number'] }}
                    </div>
                    <p>{{ $s['description'] }}</p>
                </div>
            @endforeach
        </div>

        <!-- レビュー -->
        <div class="mt-4">
            <h4 class="text-2xl font-bold mb-2">レビュー</h4>
            @foreach($recipe['reviews'] as $r)
                <div class="bg-gray-100 rounded mb-2 p-4">
                    <div class="flex items-center mb-2">
                        @for($i = 0; $i < $r['rating']; $i++)
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 text-yellow-400">
                                <path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clip-rule="evenodd" />
                            </svg>
                        @endfor
                        <p class="ml-2">{{ $r['comment'] }}</p>
                    </div>
                    <p class="text-gray-600 font-bold">{{ $r['user']['name'] }}</p>
                </div>
            @endforeach
            @if(count($recipe['reviews']) === 0)
                <p>レビューはまだありません</p>
            @endif
        </div>

    </div>
</x-app-layout>
